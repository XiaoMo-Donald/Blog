...
namespace TzCA.Web.Utilities
{
    ...
    public static class MenuItemCollection
    {
        ...
        private static void _SetMainTopMenuItems()
        {
            ...
            foreach(var item in _Context.SystemWorkPlaces.OrderBy(x=>x.BussinessCode))
            {
                menuItems.Add(new SimpleMainTopMenuItem() { Id=item.Id, Name=item.Name,URL=item.URL,SortCode=item.BussinessCode });
            }
        ...
        public static void UpdateMainTopMenuItem(SystemWorkPlace wp)
        {
            ...
            if (menuItem == null)
            {
                _MainTopMenuItems.Add(new SimpleMainTopMenuItem() { Id = wp.Id, Name = wp.Name, URL = wp.URL,SortCode=wp.BussinessCode });
            }
            else
            {
                menuItem.Name = wp.Name;
                menuItem.URL = wp.URL;
                menuItem.SortCode = wp.BussinessCode;
            }
        }
        ...
        private static void _SetSubMenuItems()
        {
            ...
            foreach(var item in wpItems)
            {
                ...
                {
                    var subMenuItem = new SimpleSubMenuItem() { Id = sItem.Id, Name = sItem.Name, SortCode = sItem.BussinessCode, ParentId = item.Id, URL = "" };
        ...
        public static void UpdateSubMenuItems(SystemWorkSection bo,Guid pId)
        {
            ...
            else
            {
                ...
                sMenuItem.URL = "";
                sMenuItem.SortCode = bo.BussinessCode;
